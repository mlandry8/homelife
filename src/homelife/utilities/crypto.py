...
def generate_cert(ip=None):
    ...
    builder = builder.subject_name(x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, 'homelife')]))
    builder = builder.issuer_name(x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, 'homelife')]))
    builder = builder.not_valid_before(datetime.datetime.today() - datetime.timedelta(1, 0, 0))
    builder = builder.not_valid_after(datetime.datetime.today() + (datetime.timedelta(1, 0, 0)*365*5))
